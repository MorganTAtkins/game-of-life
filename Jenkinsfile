...
pipeline {
    ...
    stages {
        stage('Build') {
            steps {
                echo 'Building..'
                sh "/usr/bin/mvn -e -X clean install sonar:sonar -Dskip-tests -Dsonar.host.url=http://localhost:9000"
            }
        }
        stage('Test Verify') {
            post {
            ...
            steps {
                echo 'Testing..'
                sh "/usr/bin/mvn clean verify sonar:sonar -Dsonar.host.url=http://localhost:9000"
            }
        }
    }
}